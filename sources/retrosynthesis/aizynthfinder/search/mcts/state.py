...
class MctsState:
    ...
    def extract_smiles_from_str_representation(self):
        str_rep = self.__str__()

        # Split the string representation by newline characters
        lines = str_rep.split('\n')

        # Extract the first line containing SMILES strings
        smiles_line = lines[0]

        # Extract SMILES strings from the line
        smiles_list = re.findall(r'\'(.*?)\'', smiles_line)
    ...
    def extract_smiles_from_str_representation_od(self):
