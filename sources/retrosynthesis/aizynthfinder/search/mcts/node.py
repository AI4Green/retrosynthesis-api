...
from ....aizynthfinder.context.policy.policies import ExpansionPolicy
from ....aizynthfinder.chem import TreeMolecule, deserialize_action, serialize_action, Molecule
...
class MctsNode:
    ...
    def expand(self) -> None:
        ...
        # if self.parent:
        #     print('parent position', self.parent.position)

    def find_position(self, operation, iteration):

        # for tree visualisations
        if iteration < 11:
        ...
        if not self.parent:
            self.position = [0, 0, 'root', None, self.state.extract_smiles_from_str_representation(), iterations]
        if self.parent:
            parent_depth = self.parent.position[0]
            for i in range(0, len(self.parent.children)):
                if self.parent.children[i] == self:
                    self.position = [parent_depth + 1, i+1, operation, [self.parent.position[0], self.parent.position[1]], self.state.extract_smiles_from_str_representation(), iterations]
    ...
    def create_tree_mols(self, smiles):

        tree_mol_list = []
        for i in smiles:
            temp = Chem.MolFromSmiles(str(i))
            molecule = Molecule(rd_mol=temp, smiles=str(i))
            sim_mol = TreeMolecule(parent=None, smiles=i, rd_mol=temp, sanitize=False)
            tree_mol_list.append(sim_mol)
        return tree_mol_list

    def create_mols(self, smiles):
    ...
    def find_actions(self, smiles, config):

        # selects a random molecule from the list of unsolved molecules, and returns a random action on that molecule
        # config.get_actions requires a list of tree molecules

        tree_mol_list = self.create_tree_mols(smiles)
        ran_mol = random.choice(tree_mol_list)
        mol_seq = [ran_mol]
        actions, priors = config.get_actions(mol_seq)
        return actions, ran_mol.smiles

    def perform_action(self, action):

        # Performs the action given and returns a random reactant to continue the random rollout
        reactions = action.reactants
        reactant_list = []
        for inner_tuple in reactions:
            for tree_molecule_instance in inner_tuple:
                reactant_list.append(tree_molecule_instance)

        return reactant_list

    def is_sim_terminal(self, smiles, config_max, depth, config):

        if(depth > config_max) or self.is_sim_solved(smiles, config):
            return True
        else:
            return False

    def is_sim_solved(self, smiles, config):
        mols = self.create_mols(smiles)
        stock = config.stock
        solved = 0
        for mol in mols:
            if mol in stock:
    ...
    def calculate_sim_score(self, smiles, config):
        ...
        if len(mols) == 0:
            ...
            score = in_stock/len(mols)
        return score

    def separate_solved_mols(self, smiles, config):

        mols = self.create_mols(smiles)
        solved = []
        index = 0
        for mol in mols:
            if mol in config.stock:
                solved.append(mol.smiles)
                smiles.remove(smiles[index])
                index -= 1
            index += 1
        return smiles, solved
    ...
    def promising_child(self) -> Optional["MctsNode"]:
        ...
        :return: the child
        """
        #print(self._config.C)
        def _score_and_select():
            # x = self._children_u()
            # y = self._children_eU()                                                   eUCT enhancement

            # with open('q_values.csv', 'a') as f:
            #     w = csv.writer(f, lineterminator='\n')
            #     w.writerow(x)


            # for child in self.children:
            #     if self.children[child].is_solved:
            #         scores = self._children_q()/2 + self._children_u()                mUCT-v enhancement
            #     else:
    ...
    def _children_eU(self) -> np.ndarray:
        total_visits = np.log(np.sum(self._children_visitations))
        child_visits = np.array(self._children_visitations)
        if self._config.current_iteration==0:
            phi = (1 / self._config.iteration_limit)/2
        else:
            phi = (self._config.current_iteration/self._config.iteration_limit)/2
        lamda = (self._config.C/(phi+1))
...
# loads from config.yml selecting uspto as policy

        # path_to_yaml = r'C:\Users\tonbl\test\test_env\aizynthfinder\aizynthfinder\interfaces\config.yml'
        # config_data = Configuration.from_file(path_to_yaml)
        # expansion_keys = ["uspto"]
        # expansion_policy = config_data.expansion_policy.select(*expansion_keys)
