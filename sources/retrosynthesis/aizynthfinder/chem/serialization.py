...
from sources.retrosynthesis.aizynthfinder import chem
...
class MoleculeSerializer:
    ...
    def __getitem__(self, mol: Optional[chem.Molecule]) -> Optional[int]:
        if mol is None:
            return None
    ...
    def _add_mol(self, mol: chem.Molecule) -> None:
        id_ = id(mol)
        dict_ = {"smiles": mol.smiles, "class": mol.__class__.__name__}
        if isinstance(mol, chem.TreeMolecule):
...
class MoleculeDeserializer:
    ...
    def __getitem__(self, id_: Optional[int]) -> Optional[chem.Molecule]:
        if id_ is None:
            return None
    ...
    def get_tree_molecules(
        ...
        for id_ in ids:
            obj = self[id_]
            if obj is None or not isinstance(obj, chem.TreeMolecule):
    ...
    def _create_molecules(self, store: dict) -> None:
        for id_, spec in store.items():
            ...
            kwargs = dict(spec)
            del kwargs["class"]
            self._objects[id_] = getattr(chem, cls)(**kwargs)
